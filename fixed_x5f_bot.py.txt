...
def is_active_tambah(uid, kota_name=None):
    ...
    for s in subs:
        ...
        if exp and exp > now:
            # FIX: support both old schema (kuota/used_kuota) and new schema (paket ACC)
            # Jika ada kuota, cek sisa. Jika tidak ada kuota (schema baru), anggap aktif jika expire masih ada.
            q = s.get("kuota", s.get("quota", 0))
            u = s.get("used_kuota", s.get("used", 0))
            if isinstance(u, bool):
                u = 1 if u else 0
            # Jika schema baru tanpa kuota field, anggap masih punya kuota selama expire aktif
            if q == 0 and s.get("kota") is not None:
                # Schema baru: sudah di-ACC admin, langsung anggap aktif
                if kota_name:
                    if s.get("kota","").upper() == kota_name.upper():
                        return True
                else:
                    return True
            if q - u > 0 or q == 0:  # q==0 untuk backward compat setelah fix ACC
                if kota_name:
                    if s.get("kota","").upper() == kota_name.upper():
                        return True
                else:
                    return True
    # Fallback: cek ada paket aktif apapun (expire > now)
    for s in subs:
        exp = s.get("expire")
        if isinstance(exp, str):
            try: exp = datetime.fromisoformat(exp)
...
Klik paket untuk Top Up, atau langsung pilih provinsi (Admin Unlimited):"""
            ...
            for s in subs:
                ...
                if exp and exp>now:
                    has_active_expire=True
                    qq=s.get("kuota", s.get("quota", 0))
                    uu=s.get("used_kuota", s.get("used", 0))
                    if isinstance(uu,bool): uu=1 if uu else 0
                    # FIX: Jika schema baru (sudah ACC admin) tanpa kuota, tetap anggap aktif
                    if qq == 0:
                        active=True
                        sisa+=1  # anggap masih ada sisa untuk lanjut pilih provinsi
                    elif qq - uu >0:
                        active=True
                        sisa+= qq - uu
            # FIX UTAMA: Jika ada paket aktif (expire masih berlaku) yang sudah di-ACC admin,
            # langsung ke PILIH PROVINSI, jangan suruh top up lagi
            if has_active_expire:
                active=True
            if active:
                if sisa==0:
                    sisa = len([s for s in subs if (s.get("expire") and (datetime.fromisoformat(s.get("expire")) if isinstance(s.get("expire"), str) else s.get("expire")) > now)]) if subs else 1
