        ...
        try:
            if os.path.exists(p):
                with open(p, "r", encoding="utf-8") as f:
                    ...
                    # MIGRASI STRUKTUR DATABASE BARU (List untuk langganan per kota)
                    ...
                    for uid, subs in data["langganan"].items():
                        if isinstance(subs, list):
                            for s in subs:
                                if s.get("expire"):
                                    try: s["expire"] = datetime.fromisoformat(s["expire"])
                                    except: s["expire"] = None
                        elif isinstance(subs, dict):
                             # Legacy support (kalau masih ada format lama)
                            if subs.get("expire"):
                                try: subs["expire"] = datetime.fromisoformat(subs["expire"])
                                except: subs["expire"] = None
                            data["langganan"][uid] = [subs] # Ubah ke list
...
def save_db():
    ...
    # Convert datetime di dalam list ke string
    for key in ["langganan", "langganan_cari"]:
        for k,v in db.get(key, {}).items():
            if isinstance(v, list):
                for s in v:
                    if s.get("expire") and isinstance(s["expire"], datetime):
                        s["expire"] = s["expire"].isoformat()
            elif isinstance(v, dict):
                if v.get("expire") and isinstance(v["expire"], datetime):
                    v["expire"] = v["expire"].isoformat()
...
async def cb_handler(update,context):
    ...
    if data=="kec_save":
        ...
        if added:
            if str(uid) in db["langganan"] and not is_admin(uid): 
                # Tandai pemakaian paket per-kota! (Belum diimplementasikan fully, perlu logic manual)
                pass 
            save_db()
