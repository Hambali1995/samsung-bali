...
def send_tg_message(chat_id, text, wa_number=None):
    ...
    try:
        ...
        if not r.ok:
            print(f"TG SEND ERROR chat_id={chat_id} status={r.status_code} body={r.text[:1000]}")
            return False
        result = r.json()
        if not result.get("ok"):
            print(f"TG SEND ERROR chat_id={chat_id} result={result}")
            return False
        print(f"TG SENT OK chat_id={chat_id}")
        return True
    except Exception as e:
        print(f"TG SEND EXCEPTION chat_id={chat_id}: {type(e).__name__}: {e}")
        return False
...
def whatsapp_webhook():
    try:
        ...
        print(f"WA WEBHOOK: type={data.get('typeMessage')} sender={sender_data.get('sender')} chatId={sender_data.get('chatId')} chatName={sender_data.get('chatName')}")
        ...
        if not text:
            print(f"WA WEBHOOK: text kosong. typeMessage={ttype} messageDataKeys={list(message_data.keys())}")
            return "ok",200
        ...
        for uid_str,uinfo in fresh_db.get("user_info",{}).items():
            try:
                ...
                print(f"WA FILTER uid={uid_int} wilayah={cocok_wilayah} keyword={cocok_keyword} sender={sender_number_formatted}")
                if cocok_wilayah or cocok_keyword:
                    if cocok_wilayah and uid_int not in ADMIN_IDS:
                        # Region must have its OWN active expiry.
                        region_entry = None
                        for k in kotas:
                            parts = [p.strip() for p in k.split("|")]
                            if len(parts) >= 3:
                                kab_clean = parts[1].upper().replace("KABUPATEN ", "").replace("KOTA ", "").strip()
                                kec_clean = parts[2].upper().strip()
                                if kab_clean in text_upper and (kec_clean == "SEMUA KECAMATAN" or kec_clean in text_upper):
                                    region_entry = k
                                    break
                        region_sub = fresh_db.get("user_info", {}).get(uid_str, {}).get("wilayah_langganan", {}).get(region_entry, {}) if region_entry else {}
                        exp = region_sub.get("expire") if region_sub else None
                        if isinstance(exp, str):
                            try: exp = datetime.fromisoformat(exp)
                            except: exp = None
                        if not exp or exp < datetime.now():
                            # Backward compatibility for old users: use the old global subscription
                            # only when no per-region subscription exists yet.
                            if region_entry and region_entry in fresh_db.get("user_info", {}).get(uid_str, {}).get("wilayah_langganan", {}):
                                if not cocok_keyword:
                                    continue
                            else:
                                old_sub = fresh_db.get("langganan", {}).get(uid_str)
                                old_exp = old_sub.get("expire") if old_sub else None
                                if isinstance(old_exp, str):
                                    try: old_exp = datetime.fromisoformat(old_exp)
                                    except: old_exp = None
                                if not old_exp or old_exp < datetime.now():
                                    if not cocok_keyword:
                                        continue
                ...
                print(f"WA USER PROCESS ERROR uid={uid_str}: {type(e).__name__}: {e}")
        return "ok",200
    except Exception as e:
...
def get_wilayah_langganan(uid):
    """Return per-wilayah subscription map: entry -> {paket, expire}."""
    info = db.setdefault("user_info", {}).setdefault(str(uid), {})
    wl = info.get("wilayah_langganan")
    if not isinstance(wl, dict):
        wl = {}
        info["wilayah_langganan"] = wl
    return wl


def get_region_subscription(uid, entry):
    """Get the subscription assigned specifically to a saved region."""
    wl = get_wilayah_langganan(uid)
    sub = wl.get(entry)
    if not sub:
        return None
    exp = sub.get("expire")
    ...
    if not exp or exp < datetime.now():
        return None
    return {"paket": sub.get("paket", ""), "expire": exp}


def assign_region_subscription(uid, entries):
    """Assign the user's current unused TAMBAH KOTA package to new region entries."""
    if is_admin(uid):
        return
    sub = db.get("langganan", {}).get(str(uid))
    if not sub or is_expired(sub):
        return
    wl = get_wilayah_langganan(uid)
    exp = sub.get("expire")
    if isinstance(exp, str):
    ...
    for entry in entries:
        if entry not in wl and exp:
            wl[entry] = {"paket": sub.get("paket", ""), "expire": exp}
    # One payment is one added-city entitlement. It becomes used after assignment.
    sub["used"] = True


def has_unused_tambah(uid):
    if is_admin(uid): return True
    sub = db.get("langganan", {}).get(str(uid))
    return bool(sub and not is_expired(sub))
def is_active_tambah(uid):
    return has_unused_tambah(uid)
...
async def get_status_text(uid):
    """Tampilkan status langganan per wilayah, termasuk yang sudah EXPIRED."""
    uid_str = str(uid)
    user_data = db.setdefault("user_info", {}).get(uid_str, {})
    kotas = user_data.get("kotas", []) or []
    sub = db.setdefault("langganan", {}).get(uid_str)
    sub_cari = db.setdefault("langganan_cari", {}).get(uid_str)

    if is_admin(uid):
        txt = (
            "📊 STATUS LANGGANAN\n\n"
            "👑 Status: ADMIN UNLIMITED ♾️\n"
            f"🏙️ Wilayah: {len(kotas)} kota\n"
            "⏰ Exp: Tidak terbatas\n\n"
            "✅ Semua fitur aktif!"
        )
    else:
        ...
        # Paket utama tetap ditampilkan sebagai informasi umum.
        if sub:
            exp = sub.get("expire")
            if isinstance(exp, str):
                try:
                    ...
                    exp = None
            paket = PAKET_TAMBAH.get(sub.get("paket"), {}).get("nama", "-")
            if exp:
                status_global = "❌ EXPIRED" if exp < datetime.now() else "✅ AKTIF"
                exp_str = exp.strftime("%d/%m/%Y %H:%M")
            else:
                status_global = "❌ EXPIRED"
                exp_str = "-"
            txt += f"📦 Paket TAMBAH KOTA: {paket}\n{status_global}\n⏰ Exp paket: {exp_str}\n"
        else:
            txt += "📦 Paket TAMBAH KOTA: Belum ada\n"

        # Tampilkan setiap wilayah + expired masing-masing.
        wl = get_wilayah_langganan(uid)

        # Migrasi tampilan untuk data lama: bila kota tersimpan tetapi belum
        # punya record wilayah_langganan, tetap tampilkan sebagai EXPIRED/-
        # dan jangan disembunyikan.
        region_entries = list(kotas)
        for entry in wl.keys():
            if entry not in region_entries:
                region_entries.append(entry)

        if region_entries:
            txt += f"\n📍 STATUS PER WILAYAH ({len(region_entries)}):\n"
            now = datetime.now()

            for idx, entry in enumerate(region_entries[:30], 1):
                rsub = wl.get(entry, {})
                exp = rsub.get("expire") if isinstance(rsub, dict) else None
                ...
                # Ambil nama kota/kecamatan dari format:
                # provinsi | kota/kabupaten | kecamatan
                parts = [x.strip() for x in str(entry).split("|")]
                if len(parts) >= 3:
                    lokasi = f"{parts[1]} → {parts[2]}"
                elif len(parts) == 2:
                    lokasi = f"{parts[0]} → {parts[1]}"
                else:
                    lokasi = str(entry)
        ...
        # Paket cari data tetap terpisah.
        if sub_cari:
...
async def start(update,context):
    ...
    if str(uid) not in db["user_info"]: db["user_info"][str(uid)]={"nama":nama,"username":username,"kotas":[],"custom_keywords":[],"wilayah_langganan":{}}
    else:
        ...
        if "wilayah_langganan" not in db["user_info"][str(uid)]: db["user_info"][str(uid)]["wilayah_langganan"]={}
    save_db()
...
async def cb_handler(update,context):
    ...
    if data=="kec_save":
        ...
        if added:
            assign_region_subscription(uid, added)
            save_db()
...
# IMPORTANT: do NOT recreate flask_app here; the Green API route was registered above.
