...
def is_active_cari(uid):
    ...
    if not sub:
        return False
    # Support both dict and list (fix admin activation)
...
async def start(update,context):
    ...
    save_db()
    # === TAMPILAN BARU REQUEST USER FINAL - FIX STATUS & INVITE DATE ===
    from datetime import datetime
    # Simpan waktu pertama join agar tidak berubah tiap /start
    ...
    if not first_join:
        ...
        if str(uid) not in db["user_info"]:
            db["user_info"][str(uid)] = {"nama":nama,"username":username,"kotas":[],"custom_keywords":[],"saldo":0,"first_join":first_join}
        db["user_info"][str(uid)]["first_join"] = first_join
        save_db()
    invite_date = first_join

    # Cek apakah user punya paket aktif
    has_paket_tambah = is_active_tambah(uid)
    has_paket_cari = is_active_cari(uid)
    has_paket = has_paket_tambah or has_paket_cari

    if has_paket:
        status_text = "🟢 Active"
        paket_text = "ada paket aktif"
        # ambil nama paket
        all_pkgs = []
        if str(uid) in db.get("langganan", {}):
            subs = db.get("langganan", {}).get(str(uid), [])
            if isinstance(subs, dict):
                subs = [subs]
            all_pkgs.extend(subs)
        if str(uid) in db.get("langganan_cari", {}):
            subs_c = db.get("langganan_cari", {}).get(str(uid))
            if isinstance(subs_c, dict):
                subs_c = [subs_c]
            if isinstance(subs_c, list):
                all_pkgs.extend(subs_c)
        if all_pkgs:
            paket_text = ", ".join([p.get("nama", p.get("paket","")) for p in all_pkgs[:3] if isinstance(p, dict)])
            if not paket_text:
                paket_text = "ada paket aktif"
    else:
...
❌ Ketik /batal untuk batal."""
    ...
    if data.startswith("admin_hari_"):
        ...
        try:
            ...
            await q.message.delete()
            await context.bot.send_message(chat_id=uid, text=f"✅ Durasi: {hari} HARI\n\nSekarang kirim kuota pilih provinsi:\nContoh: 3", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("3x", callback_data="admin_kuota_3"), InlineKeyboardButton("5x", callback_data="admin_kuota_5")],[InlineKeyboardButton("10x", callback_data="admin_kuota_10")]]))
        except:
            pass
        return
    ...
    if data.startswith("admin_kuota_"):
        ...
        try:
            ...
            try:
                await context.bot.send_message(chat_id=int(id_user), text=f"🎉 PAKET TAMBAH KOTA AKTIF!\n\nID: {id_user}\nDurasi: {hari} HARI\nKuota: {kuota}x pilih provinsi\nSampai: {expire.strftime('%d/%m/%Y')}\n\nBisa langsung klik TAMBAH KOTA!", reply_markup=kb_main(int(id_user)))
            except:
                pass
            await q.message.delete()
            await context.bot.send_message(chat_id=uid, text=f"✅ BERHASIL AKTIFKAN TAMBAH KOTA\nID: {id_user}\n{hari} HARI - {kuota}x", reply_markup=kb_admin_panel())
    ...
    if data.startswith("admin_cari_hari_"):
        ...
        try:
            ...
            try:
                await context.bot.send_message(chat_id=int(id_user), text=f"🎉 PAKET CARI DATA LAIN AKTIF! {hari} HARI Sampai {expire.strftime('%d/%m/%Y')}")
            except:
                pass
            await q.message.delete()
    ...
    if data.startswith("acc_") or data.startswith("dec_"):
        ...
        try:
            ...
            target_uid_int=int(''.join(filter(str.isdigit, str(target_uid))) or 0)
        ...
        if action=="acc":
            if ptype=="tambah":
                ...
                if placeholder:
                    try:
                        selected_cities = _unique_selected_cities(target_uid_int)
                        if selected_cities:
                            existing = {str(x.get("kota", "")).strip().upper() for x in db.get("langganan", {}).get(target_uid_str, [])}
                ...
                if target_uid_str not in db["langganan"]:
                    ...
                    old_val = db["langganan"][target_uid_str]
                    db["langganan"][target_uid_str] = [old_val]
                # Update or add
                city_exists=False
                for s in db["langganan"][target_uid_str]:
                    if isinstance(s, dict) and s.get("kota","").upper()==kota_target.upper() and kota_target.upper() not in {"", "UMUM", "BELUM DIPILIH"}:
                        s["expire"]=expire
                        s["paket"]=pkey
                        s["kuota"]=p.get("kuota",3)
                        s["quota"]=p.get("kuota",3)
                        s["used_kuota"]=0
                        s["used"]=False
                        s["hari"]=p["hari"]
                        s["nama"]=p["nama"]
                ...
                if not city_exists:
                    db["langganan"][target_uid_str].append({
                        "kota": kota_target if kota_target else "BELUM DIPILIH",
                        "paket": pkey,
                        "nama": p["nama"],
                        "expire": expire,
                        "kuota": p.get("kuota",3),
                ...
                try:
                    ...
                    try:
                        await q.message.edit_text(text=(q.message.text or q.message.caption or "")+f"\n\n✅ DISETUJUI - Aktif sampai {expire.strftime('%d/%m/%Y')}", reply_markup=None)
                    except:
                        pass
                try:
                    ...
                    logger.warning(f"Gagal notif user {target_uid_int}: {e}")

            else:  # cari
                p=PAKET_CARI.get(pkey,PAKET_CARI["1minggu"])
                expire=datetime.now()+timedelta(days=p["hari"])
                db.setdefault("langganan_cari", {})
                # Support both dict and list - simpan sebagai dict agar kompatibel lama, tapi is_active_cari sudah support list
                db["langganan_cari"][target_uid_str]={"paket":pkey,"expire":expire,"nama":p["nama"],"hari":p["hari"]}
                save_db()
                ...
                try:
                    ...
                    try:
                        await q.message.edit_text(text=(q.message.text or q.message.caption or "")+f"\n\n✅ DISETUJUI - {p['nama']}", reply_markup=None)
                    except:
                        pass
                try:
                    await context.bot.send_message(chat_id=target_uid_int, text=f"✅ TOP UP CARI DATA DISETUJUI ✅\n\n📦 Paket {p['nama']} aktif sampai {expire.strftime('%d/%m/%Y')}\n⏰ Durasi: {p['hari']} hari\n🎉 Sekarang bisa pakai 🔍 CARI DATA LAINNYA!\n\n👇 Klik di bawah:", reply_markup=kb_lanjut_cari_data())
                except Exception as e:
                    logger.warning(f"Gagal notif cari {target_uid_int}: {e}")
        else:
            try:
                ...
                try:
                    ...
                    pass
            try:
                await context.bot.send_message(chat_id=target_uid_int, text="❌ Top Up DITOLAK admin. Hubungi @Hambali1995")
            except:
                pass
...
async def text_handler(update,context):
    ...
    if mode == 'admin_tambah_kota_id':
        try:
            ...
            await update.message.reply_text(f"✅ ID Target: {id_user}\n\nKirim durasi: Contoh 30 HARI", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("30 HARI", callback_data="admin_hari_30"), InlineKeyboardButton("60 HARI", callback_data="admin_hari_60")]]))
        except Exception as e:
    ...
    if mode == 'admin_tambah_kota_hari':
        try:
            import re as re2
            hari = int(re2.sub(r'[^0-9]', '', text))
            if hari <=0:
                hari=30
            context.user_data['admin_hari'] = hari
            context.user_data['mode'] = 'admin_tambah_kota_kuota'
            await update.message.reply_text(f"✅ Durasi: {hari} HARI\nKirim kuota: Contoh 3 (3x pilih provinsi)", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("3x", callback_data="admin_kuota_3"), InlineKeyboardButton("5x", callback_data="admin_kuota_5")]]))
        except:
            await update.message.reply_text("❌ Format hari salah! Contoh 30")
        return

    if mode == 'admin_tambah_kota_kuota':
        try:
            import re as re2
            kuota = int(re2.sub(r'[^0-9]', '', text))
            if kuota <=0:
                kuota=3
            id_user = context.user_data.get('admin_target_id')
            hari = context.user_data.get('admin_hari', 30)
            ...
            save_user_package_permanent(id_user, f"{hari}hari_{kuota}x", "tambah")
            try:
                await context.bot.send_message(chat_id=int(id_user), text=f"🎉 PAKET TAMBAH KOTA AKTIF! {hari} HARI {kuota}x")
            except:
                pass
            await update.message.reply_text(f"✅ BERHASIL TAMBAH KOTA {id_user} {hari} HARI {kuota}x", reply_markup=kb_admin_panel())
            context.user_data['mode'] = None
            context.user_data.pop('admin_target_id', None)
            context.user_data.pop('admin_hari', None)
        except Exception as e:
    ...
    if mode == 'admin_tambah_cari_id':
        try:
            ...
            await update.message.reply_text(f"✅ ID Target: {id_user}\nKirim durasi: Contoh 30 HARI", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("30 HARI", callback_data="admin_cari_hari_30")]]))
        except Exception as e:
            await update.message.reply_text(f"❌ Error: {e}")
        return
    ...
    if mode == 'admin_tambah_cari_hari':
        try:
            ...
            save_db()
            save_user_package_permanent(id_user, f"cari_{hari}hari", "cari")
            await update.message.reply_text(f"✅ BERHASIL CARI DATA LAIN {id_user} {hari} HARI", reply_markup=kb_admin_panel())
            context.user_data['mode'] = None
            context.user_data.pop('admin_target_id', None)
        except Exception as e:
            await update.message.reply_text(f"❌ Error: {e}")
...
⚠️ Perhatian : untuk tetap waspada dan berhati-hati disarankan untuk rekber, terimakasih.sumber: https://t.me/Aakiwkiw_bot 🙏"""
    ...
    if context.user_data.get("awaiting_cari_lainnya"):
        ...
        for h in reversed(history[-10000:]):
            ...
            if query in txt_upper:
                hasil.append(h)
        # FIX: Tampilkan SEMUA pengirim tanpa batas 3 per nomor dan tanpa batas 50
        # Sesuai request user: agar tidak 1 pengirim saja tapi semua pengirim ditampilkan
