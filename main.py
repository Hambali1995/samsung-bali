...
def save_pantauan_backup(numbers):
    try:
        data = list(dict.fromkeys(numbers))
        ...
        try:
            ...
            logger.warning(f"PANTAUAN PERSISTENT SAVE WARN: {e}")
    except Exception as e:
        logger.error(f"PANTAUAN BACKUP SAVE ERROR: {e}")
...
def load_pantauan_from_supabase():
    ...
    try:
        ...
        try:
            url2 = f"{SUPABASE_URL}/rest/v1/blacklist"
            params2 = {"select": "number", "order": "id.asc", "limit": "10000"}
            r2 = requests.get(url2, headers=supabase_headers(), params=params2, timeout=15)
            r2.raise_for_status()
            rows2 = r2.json()
            return [row.get("number") for row in rows2 if row.get("number")]
        except:
            return None

def add_pantauan_to_supabase(number):
    # Selalu save ke backup file juga
    try:
        existing_backup = load_pantauan_backup()
        if number not in existing_backup:
            existing_backup.append(number)
            save_pantauan_backup(existing_backup)
    except:
        pass
    if not supabase_enabled():
        return True
    try:
        ...
        if r.status_code in [200,201,204,409]:
            return True
        logger.warning(f"PANTAUAN ADD WARN {number}: {r.status_code} {r.text[:200]}")
        return True
    except Exception as e:
        logger.warning(f"PANTAUAN ADD WARN {number}: {e}")
        return True
...
def remove_pantauan_from_supabase(number):
    ...
    try:
        ...
        try:
            url2 = f"{SUPABASE_URL}/rest/v1/blacklist"
            params2 = {"number": f"eq.{number}"}
            requests.delete(url2, headers=supabase_headers(), params=params2, timeout=10)
        except:
            pass
        return True
    except Exception as e:
        logger.warning(f"PANTAUAN REMOVE WARN {number}: {e}")
        return True

def get_all_pantauan():
    # 1. Supabase
    try:
        bl_new = load_pantauan_from_supabase()
    except:
        bl_new = None
    # 2. db memory
    local = []
    local.extend(db.get("pantauan", []) or [])
    local.extend(db.get("blacklist", []) or [])
    # 3. backup file
    backup = load_pantauan_backup()
    # Gabung semua
    combined = []
    if bl_new:
        combined.extend(bl_new)
...
def add_blacklist(num):
    ...
    ok = add_pantauan_to_supabase(clean)
        ...
        if fresh_db is None:
            try:
                ...
                fresh_db = {
                    ...
                    "pantauan": db.get("pantauan", [])
                }
        
        # === CEK NO PANTAUAN - ANTI FORWARD (TRANSAKSI AMAN) ===
        try:
            pantauan_list = get_all_pantauan()
        except:
            pantauan_list = []
        pantauan_list = list(pantauan_list or []) + list(fresh_db.get("blacklist", []) or []) + list(fresh_db.get("pantauan", []) or [])
...
def is_active_tambah(uid, kota_name=None):
    ...
    for s in subs:
        ...
        if exp and exp > now:
            ...
            if isinstance(s.get("used"), bool) and s.get("used") and u==0:
                u = 1
            if q == 0:
                pkey = s.get("paket", "1minggu")
                q = PAKET_TAMBAH.get(pkey, {"kuota": 1}).get("kuota", 1)
            if q - u > 0:
                if kota_name:
...
def assign_subscription_to_selected_city(uid, kota):
    """Hubungkan paket TAMBAH KOTA aktif dan kurangi kuota setiap pilih kota baru."""
    ...
    for sub in subs:
        ...
        if exp and exp > now:
            q = sub.get("kuota", sub.get("quota", 0))
            u = sub.get("used_kuota", 0)
            if isinstance(sub.get("used"), bool) and sub.get("used") and u==0:
                u=1
            if q == 0:
                pkey = sub.get("paket", "1minggu")
                q = PAKET_TAMBAH.get(pkey, {"kuota": 2}).get("kuota", 2)
                sub["kuota"] = q
            if u < q:
                sub["used_kuota"] = u + 1
                sub["used"] = True
                existing_upper = str(sub.get("kota", "")).strip().upper()
                if existing_upper in {"", "TIDAK ADA KOTA", "UMUM", "TIDAK DIKETAHUI", "PAKET AKTIF", "BELUM DIPILIH"}:
                    sub["kota"] = kota
                return True
    return False
        ...
        if subs:
            ...
            for s in subs:
                ...
                if exp and exp>now:
                    ...
                    if isinstance(s.get("used"), bool) and s.get("used") and u==0:
                        u=1
                    sisa = max(0, q - (u if isinstance(u,int) else 0))
                    nama_paket = PAKET_TAMBAH.get(s.get('paket'),{}).get('nama', s.get('paket','-'))
                    # Tampilkan lebih jelas sisa kuota
                    if k.upper() in {"BELUM DIPILIH", "UMUM", "TIDAK ADA KOTA"}:
                        active_kota.append(f"• {nama_paket} - Sisa {sisa}/{q} kota (Exp: {exp.strftime('%d/%m/%Y')})")
                    else:
                        active_kota.append(f"• {nama_paket} ({k}) - Sisa {sisa}/{q} (Exp: {exp.strftime('%d/%m/%Y')})")
            if active_kota:
                langganan_str = "\n".join(active_kota)
                status_icon = "🟢 Aktif"
                if active_kuota > 0:
                    exp_str = f"✅ SISA KUOTA: {active_kuota} kota lagi (Terpakai {used_kuota}/{total_kuota})"
                else:
                    exp_str = f"❌ KUOTA HABIS - Silahkan Top Up lagi! (Terpakai {used_kuota}/{total_kuota})"
            else:
                langganan_str = "Expired / Kuota habis"
                status_icon = "🔴 Tidak aktif"
                exp_str = "❌ Habis - Top Up lagi"
        else:
            langganan_str = "Belum ada paket"
            status_icon = "🔴 Tidak aktif"
            exp_str = "-"
...
🏙️ Total Wilayah Dipilih : {len(kotas)} kota

💡 Tip: Kuota = jumlah kota yang bisa ditambah. 1 kota = 1 kuota terpakai."""
    return txt
...
✅ Kuota aktif: {sisa}x pilih provinsi tersisa (dari total {total_kuota})
Paket kamu sudah di-ACC admin, silahkan lanjut pilih provinsi:

👇 Klik provinsi di bawah untuk langsung pilih KOTA:"""
    ...
    if data.startswith("acc_") or data.startswith("dec_"):
        ...
        if action=="acc":
            if ptype=="tambah":
                ...
                if not city_exists:
                    # FIX: Simpan kuota juga biar tidak dianggap habis di menu_tambah_kota
                    if kota_target.strip().upper() in {"", "TIDAK ADA KOTA", "UMUM", "TIDAK DIKETAHUI", "PAKET AKTIF"}:
...
async def foto_handler(update,context):
    ...
    # FIX: baca paket sesuai yang dipilih user
    paket_key = None
    paket_type = None
    if "pending_paket_tambah" in context.user_data:
        paket_key = context.user_data.get("pending_paket_tambah")
        paket_type = "tambah"
    elif "pending_paket_cari" in context.user_data:
...
async def cmd_cek_pantauan_debug(update, context):
    ...
    txt = f"🔧 DEBUG PANTAUAN V2 FINAL\nSupabase: {len(sup) if sup is not None else 'None/Error'} \nBackup file: {len(backup)}\nMemory pantauan: {len(mem)}\nMemory blacklist: {len(bl)}\nGabungan get_all: {len(all_p)}\n\nFile exists: {os.path.exists(PANTAUAN_FILE)} {os.path.exists(PANTAUAN_FILE_PERSISTENT)}"
    await update.message.reply_text(txt, reply_markup=kb_main(uid))
