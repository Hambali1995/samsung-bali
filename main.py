...
def kb_hasil_cari(clean_number=None):
    ...
    rows.append([InlineKeyboardButton("🔎 CARI DATA LAGI", callback_data="menu_cari_data")])
...
♾️ Unlimited - Rp 1.000.000"""
        ...
        # Jika ini dari tombol CARI DATA LAGI, kasih teks simple sesuai request
        if q and data=="menu_cari_data":
            txt="SILAHKAN KETIKAN KOTA MANA YANG AKAN DI CARI 🙏"
        else:
...
❌ Ketik /batal untuk batal."""
    ...
    if data=="menu_blacklist":
        try:
            ...
            bl = db.get("blacklist",[])
            # pagination: page from callback? default 0
            page = 0
            if "_" in data and data.count("_")>=2:
                pass # menu_blacklist no page, handled by separate callback below

            txt="🚫 NO BLACKLIST - SEMUA NOMOR\n\n"
            txt+="Nomor yang sudah tercatat di database kami bertujuan untuk berhati-hati dalam transaksi ke nomor tersebut.\n"
            txt+="Mohon maaf jika bot saya salah mencatat nomor tersebut 🙏\n\n"
            if not bl:
                txt+="📭 Belum ada nomor blacklist\n📊 Total: 0 nomor"
                await context.bot.send_message(chat_id=uid, text=txt, reply_markup=kb_blacklist_menu())
            else:
                # Show page 0 immediately, 20 per page to avoid Telegram limit
                per_page = 30
                total_pages = (len(bl) + per_page -1)//per_page
                display = bl[0:per_page]
                txt+=f"📊 Total: {len(bl)} nomor | Hal 1/{total_pages}\n\n"
                for i,n in enumerate(display,1):
                    txt+=f"{i}. {n}\n"
                ...
                if total_pages>1:
                    nav = []
                    if total_pages>1:
                        nav.append(InlineKeyboardButton("➡️ Next", callback_data="blacklist_page_1"))
                    if nav:
                        kb_rows.append(nav)
                kb_rows.append([InlineKeyboardButton("🔍 CARI NO BLACKLIST", callback_data="cari_blacklist")])
                kb_rows.append([InlineKeyboardButton("🏠 KEMBALI MENU UTAMA", callback_data="back_main")])
                await context.bot.send_message(chat_id=uid, text=txt, reply_markup=InlineKeyboardMarkup(kb_rows))
        except Exception as e:
            logger.error(f"menu_blacklist error: {e}")
            try:
                await context.bot.send_message(chat_id=uid, text=f"🚫 NO BLACKLIST\nTotal: {len(db.get('blacklist',[]))} nomor", reply_markup=kb_blacklist_menu())
            except: pass
        return
    if data.startswith("blacklist_page_"):
        ...
        try:
            page = int(data.replace("blacklist_page_",""))
        except:
            page = 0
        bl = db.get("blacklist",[])
        per_page = 30
        total_pages = (len(bl) + per_page -1)//per_page if bl else 1
        if page<0: page=0
        if page>=total_pages: page=total_pages-1
        start = page*per_page
        display = bl[start:start+per_page]
        txt=f"🚫 NO BLACKLIST - SEMUA NOMOR\n\n📊 Total: {len(bl)} nomor | Hal {page+1}/{total_pages}\n\n"
        for i,n in enumerate(display, start+1):
            txt+=f"{i}. {n}\n"
        kb_rows=[]
        nav=[]
        if page>0:
            nav.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"blacklist_page_{page-1}"))
        if page < total_pages-1:
            nav.append(InlineKeyboardButton("➡️ Next", callback_data=f"blacklist_page_{page+1}"))
        if nav:
            kb_rows.append(nav)
        kb_rows.append([InlineKeyboardButton("🔍 CARI NO BLACKLIST", callback_data="cari_blacklist")])
        kb_rows.append([InlineKeyboardButton("🏠 KEMBALI MENU UTAMA", callback_data="back_main")])
...
⚠️ Perhatian : untuk tetap waspada dan hati-hati disarankan untuk rekber, terimakasih.Sumber: https://t.me/Aakiwkiw_bot 🙏"""
                try:
                    if clean:
                        ...
                        await context.bot.send_message(chat_id=uid, text=msg, reply_markup=kb_cari_data_lain())
                except Exception as e:
                    logger.error(f"send hasil cari error: {e}")
                await asyncio.sleep(1)
            await update.message.reply_text(f"✅ Selesai menampilkan {len(hasil)} data untuk {text.upper()}", reply_markup=kb_cari_data_lain())
        return
