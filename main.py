    ...
    if data=="menu_blacklist":
        ...
        try:
            ...
            if not bl:
                ...
                return
            header = f"🚫 NO BLACKLIST - SEMUA NOMOR\n\nNomor yang sudah tercatat di database kami bertujuan untuk berhati-hati dalam transaksi ke nomor tersebut.\nMohon maaf jika bot salah mencatat 🙏\n\n📊 Total: {len(bl)} nomor\n\n"
            per_msg = 50
            chunks = [bl[i:i+per_msg] for i in range(0, len(bl), per_msg)]
            txt = header
            for idx, num in enumerate(chunks[0], 1):
                txt += f"{idx}. {num}\n"
            if len(chunks) == 1:
                kb = InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔍 CARI NO BLACKLIST", callback_data="cari_blacklist")],
                ...
                for chunk_idx, chunk in enumerate(chunks[1:], start=2):
                    txt2 = f"📋 Lanjutan {chunk_idx}/{len(chunks)} (Total {len(bl)} nomor)\n\n"
                    for num in chunk:
                        counter += 1
                        txt2 += f"{counter}. {num}\n"
                    if chunk_idx == len(chunks):
                        ...
                        await context.bot.send_message(chat_id=uid, text=txt2, reply_markup=kb)
                    else:
                        await context.bot.send_message(chat_id=uid, text=txt2)
                    await asyncio.sleep(0.3)
        except Exception as e:
            logger.error(f"menu_blacklist ALL error: {e}")
            try:
                bl = db.get("blacklist",[])
                txt = f"🚫 NO BLACKLIST\n📊 Total: {len(bl)} nomor\n\n"
                for i,n in enumerate(bl,1):
                    txt+=f"{i}. {n}\n"
                    if len(txt)>3500:
                        await context.bot.send_message(chat_id=uid, text=txt)
                        txt=""
                ...
                await context.bot.send_message(chat_id=uid, text="❌ Gagal tampilkan blacklist", reply_markup=kb_back_main_only())
        return
