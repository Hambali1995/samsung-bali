...
def send_tg_message(chat_id, text, wa_number=None):
    ...
    try:
        ...
        if not r.ok:
            print(f"TG SEND ERROR chat_id={chat_id} status={r.status_code} body={r.text[:500]}")
...
def whatsapp_webhook():
    try:
        ...
        print(f"WA WEBHOOK: {json.dumps(data, indent=2)[:500]}")
        
        # Ambil data pengirim dengan lebih baik
        ...
        if not sender_number_formatted:
            sender_number_formatted = sender_data.get("chatId", "").split("@")[0]
        
        # Ambil nama grup dan pengirim
        ...
        # Ambil teks pesan dari berbagai kemungkinan format
        text = ""
        ...
        if ttype == "textMessage":
            ...
            text = "🎵 *Pesan Suara* (klik untuk mendengarkan)"
        elif ttype == "videoMessage":
            text = "🎬 *Pesan Video* (klik untuk melihat)"
        else:
        ...
        # Jika masih kosong, coba alternatif
        if not text:
            for key in message_data:
                if "text" in key.lower():
                    msg_data = message_data.get(key, {})
                    if isinstance(msg_data, dict):
                        for subkey in msg_data:
                            if "text" in subkey.lower() or "message" in subkey.lower():
                                text = str(msg_data.get(subkey, ""))
                                break
            if not text:
                print(f"WA WEBHOOK: text kosong. typeMessage={ttype}")
                return "ok", 200
        ...
        print(f"WA WEBHOOK: sender={sender_number_formatted}, text={clean_text[:100]}")
        ...
        if is_blacklisted:
            print(f"WA: Nomor {sender_number_formatted} di blacklist, skip")
            return "ok", 200
        ...
        for uid_str, uinfo in fresh_db.get("user_info", {}).items():
            try:
                ...
                # Cek status langganan untuk filter wilayah
                if is_match:
                    ...
                    if sub:
                        ...
                        if exp and isinstance(exp, datetime) and exp < datetime.now():
                            # Expired, tapi tetap kirim jika ada keyword match
                            if not is_keyword_match:
                                continue
                
                # Kirim notifikasi jika cocok
                if is_match or is_keyword_match:
                    ...
                    print(f"WA: Notifikasi terkirim ke {uid_int} untuk match {match_type}")
