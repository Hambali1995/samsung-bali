...
def is_expired(sub):
    if not sub:
        return True
    # Jika list, cek apakah ada satu saja yang masih aktif
...
async def start(update,context):
    ...
    save_db()
    # === TAMPILAN BARU REQUEST USER FINAL ===
    from datetime import datetime
    invite_date = datetime.now().isoformat()
    paket_text = "belum ada paket"
    if str(uid) in db.get("langganan", {}) or str(uid) in db.get("langganan_cari", {}):
...
Kirim foto bukti transfer di sini"""
    ...
    if data=="menu_cari_data":
        try:
            ...
            pass
        # FIX: Pakai is_active_cari yang sudah support list & dict agar admin activation langsung bisa
...
❌ Ketik /batal untuk batal."""
    ...
    if data.startswith("acc_") or data.startswith("dec_"):
        # FIX: SETUJU/TOLAK harus selalu respon - jangan silent fail
        if not is_admin(uid):
            try:
        ...
        try:
            ...
            pass
        try:
            # Parse callback data: acc_tambah_7962377902_1bulan atau acc_cari_7962377902_1bulan
            # Format: action_ptype_uid_pkey
            parts_raw = data.split("_")
            action = parts_raw[0]  # acc atau dec
            ptype = parts_raw[1]   # tambah atau cari
            # uid bisa panjang, pkey bisa mengandung underscore, jadi ambil dari belakang
            # Cari uid: setelah ptype, angka panjang adalah uid
            # Contoh: acc_tambah_8877282096_1bulan -> ptype=tambah, uid=8877282096, pkey=1bulan
            # Contoh: acc_tambah_8877282096_30hari_3x -> pkey=30hari_3x
            remaining = "_".join(parts_raw[2:])
            # Pisahkan uid dan pkey: uid adalah angka di awal remaining
            import re as _re
            ...
            if action == "acc":
                if ptype == "tambah":
                    ...
                    expire = datetime.now() + timedelta(days=p.get("hari",30))
                    # Ambil kota dari caption jika ada
                    ...
                    # Bersihkan BELUM DIPILIH lama biar tidak numpuk
                    db["langganan"][target_uid_str] = [x for x in db["langganan"][target_uid_str] if not (isinstance(x, dict) and x.get("kota")=="BELUM DIPILIH" and x.get("paket")==pkey)]
                    ...
                    try:
                        ...
                        pass
                    # Edit pesan admin biar tau sudah disetujui
                    try:
                        if q.message.photo:
                            ...
                            await q.message.edit_text(text=(q.message.text or "")+f"\n\n✅ DISETUJUI - {p.get('nama')}", reply_markup=None)
                    except Exception as e:
                        logger.warning(f"edit caption fail: {e}")
                    # Notif ke user
                    try:
                        await context.bot.send_message(chat_id=target_uid_int, text=f"✅ TOP UP DISETUJUI ✅\n\n📦 Paket {p.get('nama')} aktif sampai {expire.strftime('%d/%m/%Y')}\n⏰ {p.get('hari',30)} hari - {p.get('kuota',3)}x pilih provinsi\n\n🎉 Bisa langsung klik 🌍 TAMBAH KOTA!", reply_markup=kb_main(target_uid_int))
                    except Exception as e:
                        logger.warning(f"notif tambah gagal {target_uid_int}: {e}")

                else:  # cari
                    p = PAKET_CARI.get(pkey, PAKET_CARI.get("1bulan", {"nama":"CARI 1 BULAN","hari":30}))
                    expire = datetime.now() + timedelta(days=p.get("hari",30))
                    db.setdefault("langganan_cari", {})
                    # Simpan sebagai dict (kompatibel) - is_active_cari sudah support list & dict
                    db["langganan_cari"][target_uid_str] = {
                        "paket": pkey,
                        "nama": p.get("nama", pkey),
                        "expire": expire,
                        "hari": p.get("hari",30)
                    ...
                    try:
                        if q.message.photo:
                            await q.message.edit_caption(caption=(q.message.caption or "")+f"\n\n✅ DISETUJUI CARI - {p.get('nama')} sampai {expire.strftime('%d/%m/%Y')}", reply_markup=None)
                        else:
                            await q.message.edit_text(text=(q.message.text or "")+f"\n\n✅ DISETUJUI CARI", reply_markup=None)
                    except:
                        pass
                    try:
                        await context.bot.send_message(chat_id=target_uid_int, text=f"✅ TOP UP CARI DATA DISETUJUI ✅\n\n📦 Paket {p.get('nama')} aktif sampai {expire.strftime('%d/%m/%Y')}\n⏰ {p.get('hari',30)} hari\n\n🎉 Sekarang bisa pakai 🔍 CARI DATA LAIN!", reply_markup=kb_main(target_uid_int))
                    except Exception as e:
                        logger.warning(f"notif cari gagal {target_uid_int}: {e}")
            else:  # dec / TOLAK
                try:
                    if q.message.photo:
            ...
            logger.error(f"SETUJU/TOLAK ERROR: {e}", exc_info=True)
            try:
                await q.message.reply_text(f"❌ Error proses SETUJU/TOLAK: {e}")
            except:
                pass
        return
