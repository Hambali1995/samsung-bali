...
def get_welcome_text(uid, user):
    ...
    username_disp = username if username and username != "-" else "-"
    invite = info.get("created_at") or info.get("join_date") or datetime.now().isoformat()
    invite_str = invite if isinstance(invite, str) else str(invite)
    try:
    ...
    paket_text = ", ".join(paket_list) if has_paket else "belum ada paket"
...
async def start(update,context):
    ...
    if str(uid) not in db["user_info"]:
        ...
        db["user_info"][str(uid)]["nama"]=nama
        db["user_info"][str(uid)]["username"]=username
