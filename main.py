...
def save_db():
    # === ANTI HILANG V3 - HEMAT EGRESS 99% ===
    # Jangan save kalau DB kosong tapi di Supabase ada isinya (cegah overwrite hilang)
    if len(db.get("user_info",{}))==0 and len(db.get("langganan",{}))==0 and len(db.get("langganan_cari",{}))==0:
        try:
            ...
            if old and (len(old.get("user_info",{}))>0 or len(old.get("langganan",{}))>0):
                logger.warning("⛔ SAVE DITAHAN! DB kosong tapi Supabase ada isinya - MENCEGAH DATA HILANG")
                return False
        except:
            pass

    # 1. SELALU SIMPAN KE FILE LOKAL DULU (paling cepat, anti hilang kalau Supabase down)
    ...
    # 2. SYNC PAKET KE TABEL PERMANEN KECIL (ini yang ANTI HILANG walau Railway dihapus)
    # Ini cuma 1KB, bukan 5MB, jadi gak jebol egress 18GB lagi
    try:
        if supabase_enabled():
            # Tambah
    ...
    # 3. SIMPAN BLOB BESAR HANYA KALAU PERLU (hemat egress 99%)
    # Jangan setiap /start, tapi hanya setiap 10 menit atau kalau admin
    ...
    import time
    now = time.time()
    # Hanya save blob besar kalau sudah 10 menit dari save terakhir
    if now - _last_big_save > 600:  # 600 detik = 10 menit
        remote_ok = save_db_to_supabase(tmp)
        if remote_ok:
            _last_big_save = now
            logger.info("✅ Big save ke Supabase berhasil - hemat egress")
        return remote_ok
    else:
        # Skip big save, tapi paket kecil sudah kesave di tabel permanen
        return True
...
def load_wa_history_from_supabase():
    ...
    try:
        ...
        # Ambil semua history, limit 10000 biar gak jebol egress
...
def save_wa_history_to_supabase(h):
    ...
    try:
        url = f"{SUPABASE_URL}/rest/v1/{WA_HISTORY_TABLE}"
        # Simpan sebagai 1 baris jsonb besar (last 10000)
...
def load_wa_history():
    # 1. Coba dari Supabase dulu (ANTI HILANG)
    sup = load_wa_history_from_supabase()
    if sup is not None and len(sup) > 0:
        # sync ke file lokal juga biar cepat
        try:
...
def save_wa_history(h):
    try:
        ...
        # Async save ke Supabase biar gak lambat
        def _bg():
...
Silahkan pilih paket yang mau diaktifkan untuk user:

1️⃣ *TAMBAH KOTA* - User bisa pilih provinsi
2️⃣ *CARI DATA LAIN* - User bisa cari history WA", parse_mode='Markdown', reply_markup=kb)
        return
    ...
    if data=="admin_pilih_paket_tambah_kota":
        ...
        await context.bot.send_message(chat_id=uid, text="🌍 *AKTIFKAN PAKET TAMBAH KOTA*

Kirim ID USER yang mau diaktifkan:
Contoh: `8877623904`

Format selanjutnya akan: ID -> HARI -> KUOTA", parse_mode='Markdown', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ BATAL", callback_data="admin_menu")]]))
        return
    ...
    if data=="admin_pilih_paket_cari_data":
        ...
        await context.bot.send_message(chat_id=uid, text="🔎 *AKTIFKAN PAKET CARI DATA LAIN*

Kirim ID USER yang mau diaktifkan:
Contoh: `8877623904`", parse_mode='Markdown', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ BATAL", callback_data="admin_menu")]]))
        return
        try: await q.message.delete()
        except: pass
        context.user_data['mode'] = 'tambah_paket_user'
        await context.bot.send_message(chat_id=uid, text="📦 *TAMBAH PAKET USER*\n\nSilahkan kirim data dengan format:\n\n`ID USER | KOTA | KECAMATAN | PAKET | KUOTA HARI`\n\nContoh:\n`7962377902 | Serang | Ciruas | CARI DATA LAIN | 30`", parse_mode='Markdown', reply_markup=kb_admin_panel())
        return
...
Contoh: `8877623904`", parse_mode='Markdown', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ BATAL", callback_data="admin_menu")]]))
        ...
        context.user_data['mode'] = 'tambah_paket_user'
        await context.bot.send_message(chat_id=uid, text="📦 *TAMBAH PAKET USER*\n\nKirim ID Telegram admin baru.\nContoh: `123456789`", parse_mode='Markdown', reply_markup=kb_admin_panel())
        return
    ...
    if data.startswith("admin_hari_"):
        ...
        try:
            ...
            context.user_data['admin_hari_selected'] = hari
            context.user_data['mode'] = 'admin_tambah_kota_hari'
            # Trigger next step by simulating text
            context.user_data['admin_hari'] = hari
            context.user_data['mode'] = 'admin_tambah_kota_kuota'
            await q.message.delete()
            await context.bot.send_message(chat_id=uid, text=f"✅ Durasi: {hari} HARI\n\nSekarang kirim kuota pilih provinsi:\nContoh: 3x", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("3x", callback_data="admin_kuota_3"), InlineKeyboardButton("5x", callback_data="admin_kuota_5")],[InlineKeyboardButton("10x", callback_data="admin_kuota_10")]]))
        except:
            pass
        return
    ...
    if data.startswith("admin_kuota_"):
        ...
        try:
            ...
            context.user_data['admin_kuota_selected'] = kuota
            # Langsung eksekusi aktivasi
            id_user = context.user_data.get('admin_target_id')
            hari = context.user_data.get('admin_hari', 30)
            from datetime import timedelta
            expire = datetime.now() + timedelta(days=hari)
            ...
            try:
                await context.bot.send_message(chat_id=int(id_user), text=f"🎉 *PAKET TAMBAH KOTA AKTIF!*\n\n🆔 ID: {id_user}\n⏰ Durasi: {hari} HARI\n🔢 Kuota: {kuota}x pilih provinsi\n📅 Aktif sampai: {expire.strftime('%d/%m/%Y')}\n\n✅ Sekarang kamu bisa langsung klik 🌍 TAMBAH KOTA!", parse_mode='Markdown', reply_markup=kb_main(int(id_user)))
            except:
                pass
            await q.message.delete()
            await context.bot.send_message(chat_id=uid, text=f"✅ BERHASIL AKTIFKAN PAKET TAMBAH KOTA\n\n🆔 User: {id_user}\n⏰ {hari} HARI\n🔢 {kuota}x pilih provinsi\n📅 Sampai: {expire.strftime('%d/%m/%Y')}", reply_markup=kb_admin_panel())
    ...
    if data.startswith("admin_cari_hari_"):
        ...
        try:
            ...
            context.user_data['admin_cari_hari_selected'] = hari
            id_user = context.user_data.get('admin_target_id')
            from datetime import timedelta
            expire = datetime.now() + timedelta(days=hari)
            ...
            db["langganan_cari"][id_user].append({
                "kota": "CARI DATA LAIN",
                "paket": f"cari_{hari}hari",
                "nama": f"CARI DATA LAIN {hari} HARI",
                "expire": expire,
                "hari": hari
            })
            save_db()
            ...
            try:
                await context.bot.send_message(chat_id=int(id_user), text=f"🎉 *PAKET CARI DATA LAIN AKTIF!*\n\n⏰ {hari} HARI\nSampai: {expire.strftime('%d/%m/%Y')}", parse_mode='Markdown')
            except:
                pass
            await q.message.delete()
            await context.bot.send_message(chat_id=uid, text=f"✅ BERHASIL AKTIFKAN CARI DATA LAIN {id_user} {hari} HARI", reply_markup=kb_admin_panel())
            context.user_data['mode'] = None
            context.user_data.pop('admin_target_id', None)
        except:
            pass
...
async def text_handler(update,context):
    ...
    # === LOGIC BARU TAMBAH PAKET USER 2 JENIS ===
    if mode == 'admin_tambah_kota_id':
        try:
            import re as re2
            id_user = re2.sub(r'[^0-9]', '', text)
            if not id_user or len(id_user) < 5:
                await update.message.reply_text("❌ ID tidak valid! Kirim ID angka contoh 8877623904")
                return
            context.user_data['admin_target_id'] = id_user
            context.user_data['mode'] = 'admin_tambah_kota_hari'
            await update.message.reply_text(f"✅ ID Target: {id_user}\n\nSekarang kirim durasi paket:\nContoh: `30 HARI` atau `30`\n\nNanti otomatis jadi 30 hari aktif", parse_mode='Markdown', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("30 HARI", callback_data="admin_hari_30"), InlineKeyboardButton("60 HARI", callback_data="admin_hari_60")],[InlineKeyboardButton("90 HARI", callback_data="admin_hari_90"), InlineKeyboardButton("❌ BATAL", callback_data="admin_menu")]]))
        except Exception as e:
    ...
    if mode == 'admin_tambah_kota_hari':
        try:
            import re as re2
            # Bisa dari button atau ketikan
            hari_text = text
            if context.user_data.get('admin_hari_selected'):
                hari_text = str(context.user_data.get('admin_hari_selected'))
                context.user_data.pop('admin_hari_selected', None)
            hari = int(re2.sub(r'[^0-9]', '', hari_text))
            if hari <=0: hari=30
            context.user_data['admin_hari'] = hari
            context.user_data['mode'] = 'admin_tambah_kota_kuota'
            await update.message.reply_text(f"✅ Durasi: {hari} HARI\n\nSekarang kirim kuota pilih provinsi:\nContoh: `3` atau `3x pilih provinsi`\n\nUser contoh 8877623904 akan bisa pilih provinsi 3x, ke-4 harus top up lagi", parse_mode='Markdown', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("3x", callback_data="admin_kuota_3"), InlineKeyboardButton("5x", callback_data="admin_kuota_5")],[InlineKeyboardButton("10x", callback_data="admin_kuota_10")]]))
        except:
            await update.message.reply_text("❌ Format hari salah! Kirim angka contoh: 30")
        return
    ...
    if mode == 'admin_tambah_kota_kuota':
        try:
            import re as re2
            kuota_text = text
            if context.user_data.get('admin_kuota_selected'):
                kuota_text = str(context.user_data.get('admin_kuota_selected'))
                context.user_data.pop('admin_kuota_selected', None)
            kuota = int(re2.sub(r'[^0-9]', '', kuota_text))
            if kuota <=0: kuota=3
            id_user = context.user_data.get('admin_target_id')
            hari = context.user_data.get('admin_hari', 30)
            from datetime import timedelta
            expire = datetime.now() + timedelta(days=hari)
            # Simpan ke DB langganan (TAMBAH KOTA)
            db.setdefault("langganan", {})
            if id_user not in db["langganan"]:
                db["langganan"][id_user] = []
            # Hapus paket lama yang sama kota BELUM DIPILIH biar tidak numpuk
            db["langganan"][id_user] = [p for p in db["langganan"][id_user] if not (isinstance(p, dict) and p.get("kota")=="BELUM DIPILIH")]
            db["langganan"][id_user].append({
                "kota": "BELUM DIPILIH",
                "paket": f"{hari}hari_{kuota}x",
            ...
            try:
                await context.bot.send_message(chat_id=int(id_user), text=f"🎉 *PAKET TAMBAH KOTA AKTIF!*\n\n🆔 ID: {id_user}\n⏰ Durasi: {hari} HARI\n🔢 Kuota: {kuota}x pilih provinsi\n📅 Aktif sampai: {expire.strftime('%d/%m/%Y')}\n\n✅ Sekarang kamu bisa langsung klik 🌍 TAMBAH KOTA dan memilih provinsi {kuota} kali!\nKe-{kuota+1} harus top up lagi yah!", parse_mode='Markdown', reply_markup=kb_main(int(id_user)))
            except Exception as e:
                logger.warning(f"Gagal notif user {id_user}: {e}")
            await update.message.reply_text(f"✅ BERHASIL AKTIFKAN PAKET TAMBAH KOTA\n\n🆔 User: {id_user}\n⏰ {hari} HARI\n🔢 {kuota}x pilih provinsi\n📅 Sampai: {expire.strftime('%d/%m/%Y')}\n\nUser sudah dapat notifikasi dan bisa langsung TAMBAH KOTA!", reply_markup=kb_admin_panel())
            context.user_data['mode'] = None
            context.user_data.pop('admin_target_id', None)
            context.user_data.pop('admin_hari', None)
        except Exception as e:
            await update.message.reply_text(f"❌ Error aktivasi: {e}")
        return
    ...
    if mode == 'admin_tambah_cari_id':
        try:
            ...
            await update.message.reply_text(f"✅ ID Target: {id_user}\n\nSekarang kirim durasi paket CARI DATA LAIN:\nContoh: `30 HARI`", parse_mode='Markdown', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("30 HARI", callback_data="admin_cari_hari_30"), InlineKeyboardButton("60 HARI", callback_data="admin_cari_hari_60")]]))
        except Exception as e:
            await update.message.reply_text(f"❌ Error: {e}")
        return
    ...
    if mode == 'admin_tambah_cari_hari':
        try:
            import re as re2
            hari_text = text
            if context.user_data.get('admin_cari_hari_selected'):
                hari_text = str(context.user_data.get('admin_cari_hari_selected'))
                context.user_data.pop('admin_cari_hari_selected', None)
            hari = int(re2.sub(r'[^0-9]', '', hari_text))
            if hari <=0: hari=30
            id_user = context.user_data.get('admin_target_id')
            from datetime import timedelta
            ...
            save_user_package_permanent(id_user, f"cari_{hari}hari", "cari")
            try:
                await context.bot.send_message(chat_id=int(id_user), text=f"🎉 *PAKET CARI DATA LAIN AKTIF!*\n\n⏰ Durasi: {hari} HARI\n📅 Sampai: {expire.strftime('%d/%m/%Y')}\n\n✅ Sekarang kamu bisa pakai menu 🔎 CARI DATA LAIN dan cari semua history WA!", parse_mode='Markdown', reply_markup=kb_main(int(id_user)))
            except:
                pass
            await update.message.reply_text(f"✅ BERHASIL AKTIFKAN CARI DATA LAIN\n\n🆔 {id_user}\n⏰ {hari} HARI\nSampai {expire.strftime('%d/%m/%Y')}", reply_markup=kb_admin_panel())
            context.user_data['mode'] = None
            context.user_data.pop('admin_target_id', None)
        except Exception as e:
            await update.message.reply_text(f"❌ Error: {e}")
...
⚠️ Perhatian : untuk tetap waspada dan berhati-hati disarankan untuk rekber, terimakasih.sumber: https://t.me/Aakiwkiw_bot 🙏"""
    ...
    if context.user_data.get("awaiting_cari_lainnya"):
        ...
        for h in reversed(history[-10000:]):
            ...
            if query in txt_upper:
                hasil.append(h)
        # === FIX: TAMPILKAN SEMUA PENGIRIM - TANPA BATAS 3 PER NOMOR, TANPA BATAS 50 ===
        # User request: agar tidak 1 pengirim saja tapi semua pengirim ditampilkan
        # Jadi hasil = semua yang match, tidak difilter
        if not hasil:
