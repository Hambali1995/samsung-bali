...
def whatsapp_webhook():
    try:
        ...
        logger.info(f"=== WA WEBHOOK RECEIVED ===")
        logger.info(f"Full data: {json.dumps(data, indent=2)[:1000]}")
        ...
        # Ambil nomor pengirim - coba berbagai source
        sender_number = ""
        ...
        logger.info(f"Sender number: {sender_number_formatted}")
        ...
        logger.info(f"TypeMessage: {ttype}")
        ...
        if ttype == "textMessage":
            ...
            # Coba ambil dari textMessageData
        ...
        if not text:
            logger.warning(f"Text kosong, typeMessage={ttype}, messageData keys: {list(message_data.keys())}")
            return "ok", 200
        ...
        logger.info(f"Text: {clean_text[:200]}")
        ...
        if is_blacklisted:
            logger.info(f"Nomor {sender_number_formatted} di blacklist, skip")
            return "ok", 200
