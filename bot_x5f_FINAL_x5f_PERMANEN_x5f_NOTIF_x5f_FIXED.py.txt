...
# ========== TABEL BARU PERMANEN - ANTI HILANG V3 ==========
USER_PACKAGES_TABLE = "user_packages"
USER_MENUS_TABLE = "user_menus"
BOT_USERS_PERM_TABLE = "bot_users_permanent"

def save_user_package_permanent(user_id, paket_key, tipe="tambah"):
    """Simpan paket user ke tabel permanen - TIDAK AKAN HILANG"""
    if not supabase_enabled():
        return False
    try:
        ...
        body = {
            "user_id": str(user_id),
            "paket_key": paket_key,
            "paket_nama": info.get("nama", paket_key),
            "tipe": tipe,
            "harga": info.get("harga"),
            "hari": info.get("hari"),
            "kuota": info.get("kuota"),
            "status": "aktif"
        }
        if info.get("hari"):
        ...
        r = requests.post(
            f"{url}?on_conflict=user_id,paket_key,tipe",
            headers={**supabase_headers(), "Prefer": "resolution=merge-duplicates,return=representation"},
            json=body,
            timeout=15
        )
        if r.status_code in [200,201]:
            logger.info(f"✅ Paket permanen disimpan: {user_id} -> {paket_key} ({tipe})")
            return True
        else:
            logger.warning(f"save permanen fail {r.status_code}: {r.text[:300]}")
            return False
    except Exception as e:
        logger.error(f"save permanen error: {e}")
        return False
...
def load_all_packages_permanent():
    ...
    try:
        ...
        for row in rows:
            uid = str(row["user_id"])
            tipe = row.get("tipe", "tambah")
            target = result_tambah if tipe == "tambah" else result_cari
            target.setdefault(uid, []).append({
                "paket": row.get("paket_key"),
                "nama": row.get("paket_nama"),
                "expire": row.get("expire"),
                "harga": row.get("harga"),
            })
        return {"tambah": result_tambah, "cari": result_cari}
    except Exception as e:
        logger.warning(f"load permanen error: {e}")
        return {"tambah": {}, "cari": {}}

def save_user_menu_permanent(user_id):
    if not supabase_enabled():
        return False
    try:
        uid = str(user_id)
        body = {
            "user_id": uid,
            "langganan": db.get("langganan", {}).get(uid, {}),
            "langganan_cari": db.get("langganan_cari", {}).get(uid, {}),
            "user_info": db.get("user_info", {}).get(uid, {}),
            "extra": {"saved_at": datetime.now().isoformat()},
        }
        url = f"{SUPABASE_URL}/rest/v1/{USER_MENUS_TABLE}"
        r = requests.post(
            f"{url}?on_conflict=user_id",
            headers={**supabase_headers(), "Prefer": "resolution=merge-duplicates,return=representation"},
            json=body,
            timeout=15
        )
        ...
        logger.error(f"save menu permanen error: {e}")
        return False
...
def load_db_with_permanent_fallback():
    ...
    if perm and (perm.get("tambah") or perm.get("cari")):
        if not remote or (len(remote.get("langganan", {})) == 0 and len(perm.get("tambah", {})) > 0):
            logger.info(f"🔄 RESTORE dari tabel permanen: {len(perm.get('tambah', {}))} tambah, {len(perm.get('cari', {}))} cari")
            if not remote:
...
def save_db():
    # ANTI HILANG V3: cegah save kosong
    ...
    # SYNC KE TABEL PERMANEN (biar semua menu tersimpan permanen)
    try:
        if supabase_enabled():
            for uid, pkgs in db.get("langganan", {}).items():
                if isinstance(pkgs, list):
                    for p in pkgs:
                        key = p.get("paket") if isinstance(p, dict) else str(p)
                        if key:
                            save_user_package_permanent(uid, key, "tambah")
            ...
            for uid, pkgs in db.get("langganan_cari", {}).items():
                if isinstance(pkgs, list):
                    for p in pkgs:
                        key = p.get("paket") if isinstance(p, dict) else str(p)
                        if key:
                            save_user_package_permanent(uid, key, "cari")
    except Exception as e:
...
db = load_db()
# FIX V3: auto-seed kosong dimatikan biar tidak menghapus data
...
async def foto_handler(update,context):
    ...
    # FIX NOTIFIKASI ADMIN: bedakan TAMBAH KOTA vs CARI DATA LAIN
    if paket_type == "tambah":
