...
def kb_admin_panel():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🚫 KELOLA BLACKLIST", callback_data="admin_blacklist_menu")],
        [InlineKeyboardButton("⬅️ KEMBALI", callback_data="back_main")],
        [InlineKeyboardButton("🏠 KEMBALI MENU UTAMA", callback_data="back_main")]
    ])
...
♾️ Unlimited - Rp 1.000.000"""
        ...
        await context.bot.send_message(chat_id=uid, text="🔎 CARI DATA LAIN\n\nSilahkan ketik nama Kota / Kecamatan yang ingin dicari:", reply_markup=kb_back_main_only())
        return
    if data=="menu_blacklist":
        ...
        if not bl:
            ...
            for n in bl[:100]:
                txt+=f"• {n}\n"
            if len(bl)>100:
                txt+=f"\n... dan {len(bl)-100} lainnya\n"
            txt+="\nGunakan tombol CARI untuk cek nomor"
    ...
    if data=="admin_status_user":
        ...
        now=datetime.now()
        text_out="📊 STATUS USER AKTIF - DETAIL\n━━━━━━━━━━━━━━\n\n"
        count=0
        for uid_str, subs in db.get("langganan", {}).items():
            ...
            for s in subs:
                ...
                if exp and exp>now:
                    ...
                    paket_info.append(f"{s.get('paket','-')} ({s.get('nama','-')}) sisa {qq-uu}/{qq} exp {exp_str}")
            if not paket_info:
                continue
            count+=1
    ...
    if data.startswith("admin_list_paket_"):
        ...
        buttons.append([InlineKeyboardButton("⬅️ KEMBALI", callback_data="admin_hapus_paket")])
        try: await q.message.delete()
        except: pass
    ...
    if data=="admin_broadcast":
        ...
        await context.bot.send_message(chat_id=uid, text="📢 MODE BROADCAST\nKirim pesan broadcast\nKetik /batal untuk batal", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Batal",callback_data="admin_menu")]]))
        return
    ...
    if data.startswith("prov_"):
        ...
        buttons.append([InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_tambah_kota")])
        buttons.append([InlineKeyboardButton("🏠 KEMBALI MENU UTAMA", callback_data="back_main")])
    ...
    if context.user_data.get("awaiting_cari_data"):
        ...
        if not hasil:
            ...
            for i,h in enumerate(hasil[:10],1):
                txt+=f"{i}. 📍 {h.get('group','-')}\n"
            await update.message.reply_text(txt, reply_markup=kb_cari_data_lain())
            for h in hasil[:5]:
                nomor=h.get("number","")
...
⚠️ Perhatian : untuk tetap waspada dan hati-hati disarankan untuk rekber, terimakasih 🙏"""
                try:
                    if clean:
                        kb=InlineKeyboardMarkup([[InlineKeyboardButton("💬 Chat di WA", url=f"https://wa.me/{clean}")],[InlineKeyboardButton("⬅️ KEMBALI", callback_data="back_main")],[InlineKeyboardButton("🏠 KEMBALI MENU UTAMA", callback_data="back_main")]])
    ...
    if context.user_data.get("awaiting_cari_lainnya"):
        ...
        if not hasil:
            ...
            await update.message.reply_text(txt, reply_markup=kb_back_main_only())
            for h in hasil[:5]:
                nomor=h.get("number","")
                clean=''.join(filter(str.isdigit,nomor))
                if clean.startswith("0"): clean="62"+clean[1:]
