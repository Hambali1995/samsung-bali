...
async def start(update,context):
    ...
    # === FIX STATUS & INVITE DATE - FINAL ===
    from datetime import datetime as _dt
    # Simpan waktu pertama join
    user_info = db.get("user_info", {}).get(str(uid), {})
    first_join = user_info.get("first_join")
    if not first_join:
        first_join = _dt.now().isoformat()
        db["user_info"][str(uid)]["first_join"] = first_join
        db["user_info"][str(uid)].setdefault("nama", nama)
        db["user_info"][str(uid)].setdefault("username", username)
        save_db()
    invite_date = first_join

    # Cek paket aktif yang sebenarnya
    has_tambah = False
    has_cari = False
    try:
        has_tambah = is_active_tambah(uid)
    except:
        has_tambah = False
    try:
        has_cari = is_active_cari(uid)
    except:
        has_cari = False
    has_paket = has_tambah or has_cari

    if has_paket:
        status_text = "🟢 Active"
        # Ambil nama paket aktif saja
        active_pkgs = []
        subs = db.get("langganan", {}).get(str(uid), [])
        if isinstance(subs, dict):
            subs = [subs]
        for s in subs:
            if isinstance(s, dict) and not is_expired(s):
                active_pkgs.append(s)
        subs_c = db.get("langganan_cari", {}).get(str(uid))
        if isinstance(subs_c, dict) and not is_expired(subs_c):
            active_pkgs.append(subs_c)
        elif isinstance(subs_c, list):
            for s in subs_c:
                if isinstance(s, dict) and not is_expired(s):
                    active_pkgs.append(s)
        if active_pkgs:
            paket_text = ", ".join([p.get("nama", p.get("paket","")) for p in active_pkgs[:3]])
        else:
