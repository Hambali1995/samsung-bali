...
def save_db():
    # ANTI HILANG: jangan save kosong
    ...
    try:
        ...
        for uid_str, uinfo in fresh_db.get("user_info", {}).items():
            try:
                ...
                # Cek admin_notif flag - jika admin nonaktif, skip
                if is_admin_user:
                    admin_info = fresh_db.get("user_info", {}).get(uid_str, {})
                    if admin_info.get("admin_notif") == False:
                        continue
                # Cek kecocokan wilayah - ADMIN JUGA DAPAT
...
def kb_admin_panel():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("👥 TAMPILKAN USER START", callback_data="admin_tampilkan_user_start")],
        [InlineKeyboardButton("🔍 CARI USER", callback_data="admin_cari_user")],
        [InlineKeyboardButton("🔔 NOTIF ADMIN ON/OFF & KOTA SAYA", callback_data="admin_notif_kota")],
...
def assign_subscription_to_selected_city(uid, kota):
    ...
    for sub in subs:
        ...
        if exp and exp > now:
            ...
            if u < q:
                sub["used_kuota"] = u + 1
                if "kota_history" not in sub:
                    sub["kota_history"] = []
                if kota not in sub["kota_history"]:
                    sub["kota_history"].append(kota)
                sub["kota"] = kota
                sub["used"] = True
...
def get_welcome_text(uid, user):
    info = db.get("user_info", {}).get(str(uid), {})
    username = getattr(user, 'username', None) or info.get("username", "-")
    if username and not username.startswith("@") and username != "-":
        username_disp = username
    else:
        username_disp = info.get("username", "-") if info.get("username") else (getattr(user, 'username', None) or "-")
    
    # Invite date
    invite = info.get("created_at") or info.get("join_date") or datetime.now().isoformat()
    if isinstance(invite, str):
        invite_str = invite
    else:
        invite_str = invite.isoformat() if hasattr(invite, 'isoformat') else str(invite)
    
    # Status
    is_aktif = is_user_id_aktif(uid)
    status_text = "🟢 Active" if is_aktif else "🔴 Not Active"
    
    # Paket info
    subs_tambah = db.get("langganan", {}).get(str(uid), [])
    if isinstance(subs_tambah, dict): subs_tambah=[subs_tambah]
    subs_cari = db.get("langganan_cari", {}).get(str(uid), [])
    if isinstance(subs_cari, dict): subs_cari=[subs_cari]
    
    paket_list=[]
    now=datetime.now()
    has_paket=False
    for s in subs_tambah:
        exp=s.get("expire")
        if isinstance(exp,str):
            try: exp=datetime.fromisoformat(exp)
            except: exp=None
        if exp and exp>now:
            ...
            if sisa>0:
                has_paket=True
                paket_list.append(f"TAMBAH KOTA {sisa}/{qq}X")
    for s in subs_cari:
        if isinstance(s, dict):
            ...
            if exp and exp>now:
                has_paket=True
                paket_list.append("CARI DATA LAIN")
    
    if has_paket:
        paket_text = ", ".join(paket_list)
    else:
        paket_text = "belum ada paket"
    
    # Nama user untuk sapaan
    nama_sapa = getattr(user, 'first_name', None) or info.get("nama", "Sahabat")
...
📥 Paket : {paket_text}
━━━━━━━━━━━━━▪️▪️▪️
🎉 Selamat datang {nama_sapa} ({uid}), untuk mengaktifkan fitur ini silahkan pilih paket yang sudah ada, tetap semangat dan jangan lupa bersyukur untuk hari ini. 🔥
━━━━━━━━━━━━━▪️▪️▪️
...
async def start(update,context):
    ...
    if str(uid) not in db["user_info"]:
        db["user_info"][str(uid)]={"nama":nama,"username":username,"kotas":[],"custom_keywords":[],"saldo":0,"created_at":now_iso,"join_date":now_iso}
    else:
        ...
        if "created_at" not in db["user_info"][str(uid)]: db["user_info"][str(uid)]["created_at"]=now_iso
        if "join_date" not in db["user_info"][str(uid)]: db["user_info"][str(uid)]["join_date"]=now_iso
    save_db()
    txt = get_welcome_text(uid, update.effective_user)
...
❌ Ketik /batal untuk batal."""
    ...
    if data=="admin_notif_kota":
        if not is_admin(uid): return
        try: await q.message.delete()
        except: pass
        info = db.get("user_info", {}).get(str(uid), {})
        kotas = info.get("kotas", [])
        notif_status = info.get("admin_notif", True)
        kota_text = "\n".join([f"• {k}" for k in kotas[:30]]) if kotas else "(Belum pilih kota)"
        if len(kotas) > 30:
            kota_text += f"\n... dan {len(kotas)-30} lainnya"
        txt = f"🔔 *NOTIFIKASI ADMIN*\n\n"
        txt += f"Status notif kota: {'✅ AKTIF' if notif_status else '❌ NONAKTIF'}\n"
        txt += f"📍 Kota yang kamu pilih ({len(kotas)}):\n{kota_text}\n\n"
        txt += f"Admin sama seperti user, akan dapat notifikasi WA jika ada pesan masuk yang cocok dengan kota kamu.\n\n"
        txt += f"Pilih aksi:"
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"{'🔕 NONAKTIFKAN NOTIF' if notif_status else '🔔 AKTIFKAN NOTIF'}", callback_data="admin_toggle_notif")],
            [InlineKeyboardButton("🌍 TAMBAH KOTA (SEPERTI USER)", callback_data="menu_tambah_kota")],
            [InlineKeyboardButton("🗑️ HAPUS SEMUA KOTA SAYA", callback_data="hapus_semua_kota")],
            [InlineKeyboardButton("🏠 KEMBALI ADMIN", callback_data="admin_menu")]
        ])
        await context.bot.send_message(chat_id=uid, text=txt, parse_mode='Markdown', reply_markup=kb)
        return

    if data=="admin_toggle_notif":
        if not is_admin(uid): return
        info = db.get("user_info", {}).setdefault(str(uid), {})
        current = info.get("admin_notif", True)
        info["admin_notif"] = not current
        save_db()
        await q.answer(f"Notif {'AKTIF' if not current else 'NONAKTIF'}", show_alert=True)
        # reload menu
        try: await q.message.delete()
        except: pass
        # call same menu again
        kotas = info.get("kotas", [])
        kota_text = "\n".join([f"• {k}" for k in kotas[:30]]) if kotas else "(Belum pilih kota)"
        txt = f"🔔 *NOTIFIKASI ADMIN*\n\nStatus notif kota: {'✅ AKTIF' if info['admin_notif'] else '❌ NONAKTIF'}\n📍 Kota ({len(kotas)}):\n{kota_text}"
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"{'🔕 NONAKTIFKAN' if info['admin_notif'] else '🔔 AKTIFKAN'}", callback_data="admin_toggle_notif")],
            [InlineKeyboardButton("🌍 TAMBAH KOTA", callback_data="menu_tambah_kota")],
            [InlineKeyboardButton("🏠 KEMBALI ADMIN", callback_data="admin_menu")]
        ])
        await context.bot.send_message(chat_id=uid, text=txt, parse_mode='Markdown', reply_markup=kb)
        return
    ...
    if data=="admin_status_user":
        ...
        now=datetime.now()
        text_out="📊 STATUS USER + SISA KUOTA\n━━━━━━━━━━━━━━━━━━━━━\n\n"
        count=0
        all_user_ids = set(list(db.get("user_info", {}).keys()) + list(db.get("langganan", {}).keys()) + list(db.get("langganan_cari", {}).keys()))
        for uid_str in all_user_ids:
            ...
            if username != "-" and not username.startswith("@"):
                username = f"@{username}"
            kotas=info.get("kotas",[])
            subs_tambah = db.get("langganan", {}).get(uid_str, [])
            if isinstance(subs_tambah, dict): subs_tambah=[subs_tambah]
            subs_cari = db.get("langganan_cari", {}).get(uid_str, [])
            if isinstance(subs_cari, dict): subs_cari=[subs_cari]
            paket_info=[]
            has_active=False
            total_sisa=0
            ...
            for s in subs_tambah:
                ...
                if exp:
                    ...
                    sisa=max(0, qq-uu)
                    kota_s=s.get("kota","-")
                    if kota_s in ["BELUM DIPILIH", "-", "UMUM", ""]:
                        kota_s = "Belum pilih"
                    total_sisa+=sisa if is_active else 0
                    total_kuota+=qq
                    total_used+=uu
                    paket_info.append(f"{status_icon} TAMBAH KOTA | {kota_s} | Sisa: {sisa}/{qq}X (pakai {uu}X) | Exp: {exp_str}")
            for s in subs_cari:
                if isinstance(s, dict):
                    exp=s.get("expire")
                    if isinstance(exp,str):
                    ...
                    if exp:
                        ...
                        total_kuota+=qq
                        total_used+=uu
                        paket_info.append(f"{status_icon} CARI DATA | Sisa: {sisa}/{qq}X | Exp: {exp_str}")
            if kotas or subs_tambah or subs_cari:
                count+=1
                if has_active:
                    header = f"✅ AKTIF - {uid_str} | {nama} | Sisa: {total_sisa}/{total_kuota}X"
                elif subs_tambah or subs_cari:
                    header = f"⏰ EXPIRED/HABIS - {uid_str} | {nama} | Terpakai: {total_used}/{total_kuota}X"
                else:
                    header = f"⚠️ NO PAKET - {uid_str} | {nama}"
                text_out+=f"{count}. {header}\n   🔗 {username}\n   🌍 Kota: {', '.join(kotas[:2]) if kotas else '-'} ({len(kotas)} kota)\n"
                if paket_info:
                    for pi in paket_info[:4]:
                        text_out+=f"      {pi}\n"
                text_out+="\n"
                if len(text_out) > 3500:
                    text_out+="\n... terpotong, cek file export\n"
                    break
        if count==0:
            text_out+="❌ Tidak ada user"
        else:
            text_out+=f"\n📊 Total: {count} user\n"
            text_out+=f"✅ = Aktif (sisa kuota masih ada)\n"
            text_out+=f"⏰ = Expired / Kuota habis 0X\n"
            text_out+=f"⚠️ = Pilih kota tapi belum diaktifkan\n"
            text_out+=f"\n💡 Topup: TAMBAH PAKET USER -> ID | hari | kuota"
    ...
    if data=="kec_save":
        ...
        if added:
            # ADMIN JUGA BISA PILIH KOTA DAN DAPAT NOTIF - FIX REQUEST USER
            assign_subscription_to_selected_city(uid, kota)  # admin juga dipanggil, unlimited
            if not is_admin(uid):
                pass  # user biasa sudah di handle di dalam fungsi
            ...
            try:
                ...
                if is_admin(uid):
                    ...
                    # Hitung sisa kuota pilih provinsi
                    subs_tmp = db.get("langganan", {}).get(str(uid), [])
                    if isinstance(subs_tmp, dict): subs_tmp=[subs_tmp]
                    sisa = 0
                    total = 0
                    for s in subs_tmp:
                        exp_tmp = s.get("expire")
                        if isinstance(exp_tmp, str):
                            try: exp_tmp = datetime.fromisoformat(exp_tmp)
                            except: exp_tmp=None
                        if exp_tmp and exp_tmp > datetime.now():
                            q = s.get("kuota", 0)
                            u = s.get("used_kuota", 0)
                            sisa += max(0, q-u)
                            total += q
                    sisa_text = f"\n🔢 Sisa kuota: {sisa}/{total}X pilih provinsi" if total>0 else ""
                    lock_text = "\n\n🔒 Kuota habis! Menu TAMBAH KOTA terkunci otomatis. Hubungi admin." if sisa==0 and total>0 else ""
                    notif_text = f"✅ BERHASIL TAMBAH KOTA\n🌍 {prov} | {kota}\n📍 {', '.join(selected[:10])}\n📊 {len(added)} kecamatan ditambahkan{sisa_text}\n⏰ {datetime.now().strftime('%d/%m %H:%M')}{lock_text}"
                    await context.bot.send_message(chat_id=uid, text=notif_text)
    ...
    if data.startswith("acc_") or data.startswith("dec_"):
        ...
        try:
            ...
            if rest.startswith("tambah_"):
                ptype="tambah"; remaining=rest[len("tambah_"):]
            elif rest.startswith("cari_"):
                ptype="cari"; remaining=rest[len("cari_"):]
            else:
                parts=data.split("_"); ptype=parts[1]; remaining="_".join(parts[2:]); action=parts[0]
            m = remaining.split("_",1)
            target_uid=m[0]; pkey=m[1] if len(m)>1 else "1minggu"
            if not pkey: pkey="1minggu"
        except Exception:
        ...
        if action=="acc":
            if ptype=="tambah":
                ...
                for s in db["langganan"][str(target_uid)]:
                    if s.get("kota", "").upper() == kota_target.upper() and kota_target.upper() not in {"", "UMUM", "BELUM DIPILIH"}:
                        s["expire"] = expire
                        s["paket"] = pkey
                        s["kuota"] = 1
                        s["used_kuota"] = 0
...
async def text_handler(update,context):
    ...
    # ===== TAMBAH PAKET USER - FLOW BARU SESUAI REQUEST - SUPPORT FORMAT BARIS BARU =====
    if mode in ['tambah_paket_tambah_kota', 'tambah_paket_cari_data', 'tambah_paket_user']:
        try:
            import re as re2
            id_user = ""
            masa_aktif = 0
            ...
            # CLEAN TEXT - support 3 format
            # Format 1: Baris baru (REQUEST TERBARU KAMU)
            # 8877282096
            # 30 hari
            # Format 2: ID | HARI
            # Format 3: ID | KOTA | KEC | PAKET | HARI
            ...
            if "|" in text_clean:
                # Format pakai pipa |
                parts = [x.strip() for x in text_clean.split("|")]
                if len(parts) >= 3:
                    # ID | HARI | KUOTA -> 8877282096 | 30 | 3
                    id_user_raw = parts[0]
                    hari_raw = parts[1]
                    kuota_raw = parts[2]
                    id_user = re2.sub(r'[^0-9]', '', id_user_raw)
                    masa_aktif = int(re2.sub(r'[^0-9]', '', hari_raw) or 0)
                    kuota_pilih = int(re2.sub(r'[^0-9]', '', kuota_raw) or 3)
                elif len(parts) == 2:
                    id_user_raw, hari_raw = parts[0], parts[1]
                    id_user = re2.sub(r'[^0-9]', '', id_user_raw)
                    masa_aktif = int(re2.sub(r'[^0-9]', '', hari_raw))
                    kuota_pilih = 3
                elif len(parts) >= 5:
                    id_user_raw, kota, kecamatan, paket_raw, hari_raw = parts[0], parts[1], parts[2], parts[3], parts[4]
                    id_user = re2.sub(r'[^0-9]', '', id_user_raw)
                    masa_aktif = int(re2.sub(r'[^0-9]', '', hari_raw))
                else:
                    # fallback
                    id_user = re2.sub(r'[^0-9]', '', parts[0])
                    # cari angka di part lain
                    for p in parts[1:]:
                        nums = re2.sub(r'[^0-9]', '', p)
                        if nums:
                            masa_aktif = int(nums)
                            break
            elif len(lines) >= 3:
                # Format 3 BARIS BARU (REQUEST TERBARU):
                # 8877282096
                # 30 hari
                # 3X pilih provinsi
                id_user_raw = lines[0]
                ...
                id_user = re2.sub(r'[^0-9]', '', id_user_raw)
                masa_aktif = int(re2.sub(r'[^0-9]', '', hari_raw) or 0)
                # ambil angka dari "3X pilih provinsi" -> 3
                kuota_match = re2.search(r'(\d+)', kuota_raw)
                if kuota_match:
                    kuota_pilih = int(kuota_match.group(1))
                else:
                    kuota_pilih = 3
                print(f"Format 3 baris terdeteksi: ID={id_user}, hari={masa_aktif}, kuota={kuota_pilih}")
            elif len(lines) >= 2:
                # Format BARIS BARU - ID di baris 1, HARI di baris 2
                # Contoh:
                # 8877282096
                # 30 hari
                id_user_raw = lines[0]
                hari_raw = lines[1]
                id_user = re2.sub(r'[^0-9]', '', id_user_raw)
                # ambil angka dari "30 hari" -> 30
                masa_aktif = int(re2.sub(r'[^0-9]', '', hari_raw))
                kuota_pilih = 3
                print(f"Format baris baru terdeteksi: ID={id_user}, hari={masa_aktif}")
            elif len(lines) == 1:
                # Cuma 1 baris, cek apakah ada angka hari juga di baris yang sama dipisah spasi
                # Contoh: "8877282096 30" atau "8877282096 30 hari"
                tokens = lines[0].split()
                if len(tokens) >= 2:
                    id_user = re2.sub(r'[^0-9]', '', tokens[0])
                    # cari angka di token selanjutnya
                    for tok in tokens[1:]:
                        nums = re2.sub(r'[^0-9]', '', tok)
                        if nums:
                            masa_aktif = int(nums)
                            break
                else:
                    # Hanya ID, default 30 hari
                    id_user = re2.sub(r'[^0-9]', '', lines[0])
                    masa_aktif = 30
            
            if not id_user:
                await update.message.reply_text(
                    "❌ Format salah!\n\n"
                    "Format yang benar (baris baru):\n"
                    "`8877282096`\n"
                    "`30 hari`\n\n"
                    "Atau format pipa:\n"
                    "`8877282096 | 30`",
            ...
            if masa_aktif == 0:
                masa_aktif = 30  # default 30 hari jika tidak terdeteksi
            
            # Validasi masa aktif minimal 1 hari, maksimal 365
            if masa_aktif < 1:
                masa_aktif = 1
            if masa_aktif > 365:
                masa_aktif = 365
            ...
            if not id_user:
                await update.message.reply_text("❌ ID tidak valid", reply_markup=kb_admin_panel())
                return

            # Tentukan tipe paket dari mode
            paket_type = context.user_data.get('paket_type', 'tambah_kota')
            if mode == 'tambah_paket_cari_data' or paket_type == 'cari_data':
                paket_type = 'cari_data'
                paket_nama = 'CARI DATA LAIN'
                paket_key = 'cari_data_lain'
            else:
                paket_type = 'tambah_kota'
                paket_nama = 'TAMBAH KOTA'
                paket_key = 'tambah_kota'

            from datetime import timedelta as td
            expire = datetime.now() + td(days=masa_aktif)
            expire_str = expire.strftime("%d/%m/%Y %H:%M")

            # Simpan ke DB sesuai tipe
            if paket_type == 'tambah_kota':
                db.setdefault("langganan", {})
                if id_user not in db["langganan"]:
                    db["langganan"][id_user] = []
                # Cek apakah sudah ada paket aktif yang sama
                ...
                # Pastikan user_info ada
                db.setdefault("user_info", {})
                if id_user not in db["user_info"]:
                    db["user_info"][id_user] = {"nama": f"User {id_user}", "kotas": [], "username": "-"}
            else:  # cari_data
                db.setdefault("langganan_cari", {})
                if id_user not in db["langganan_cari"]:
                    db["langganan_cari"][id_user] = []
                # Untuk CARI DATA LAIN, strukturnya beda - pakai dict single atau list?
                # Kita buat konsisten: simpan sebagai dict dengan expire
                # Hapus yang lama jika ada dan ganti baru
                db["langganan_cari"][id_user] = [{
                    "paket": paket_key,
                    "nama": paket_nama,
                    "kuota": 1000,
                    ...
                    "created_at": datetime.now().isoformat(),
                    "type": "cari_data"
                }] if isinstance(db["langganan_cari"].get(id_user), dict) or True else db["langganan_cari"][id_user]
                # Simpan juga sebagai list untuk kompatibilitas
                if isinstance(db["langganan_cari"][id_user], dict):
                    db["langganan_cari"][id_user] = [db["langganan_cari"][id_user]]
            ...
            # Notifikasi ke admin
            await update.message.reply_text(
                f"✅ *PAKET BERHASIL DIAKTIFKAN*\n\n"
                f"🆔 ID User: `{id_user}`\n"
                f"📦 Paket: {paket_nama}\n"
                f"⏳ Masa aktif: {masa_aktif} hari\n"
                f"📅 Expire: {expire_str}\n\n"
                f"User akan dapat notifikasi sekarang.",
                parse_mode='Markdown',
            ...
            try:
                user_nama = db.get("user_info", {}).get(id_user, {}).get("nama", "Sahabat JHT")
                notif_user = f"🎉 *PAKET AKTIF!*\n\n"
                notif_user += f"Halo {user_nama}! 👋\n\n"
                notif_user += f"✅ Paket kamu *{paket_nama}* sudah diaktifkan oleh admin!\n\n"
                notif_user += f"📦 Paket: {paket_nama}\n"
                notif_user += f"⏳ Aktif selama: {masa_aktif} hari ({masa_aktif//30} bulan {masa_aktif%30} hari)\n"
                notif_user += f"📅 Expire: {expire_str}\n\n"
                
                if paket_type == 'tambah_kota':
                    notif_user += f"🌍 Sekarang kamu bisa langsung pilih kota:\n"
                    notif_user += f"Klik menu *TAMBAH KOTA* di bawah untuk mulai! 👇\n"
                    kb_user = InlineKeyboardMarkup([
                        [InlineKeyboardButton("🌍 TAMBAH KOTA SEKARANG", callback_data="menu_tambah_kota")],
                        [InlineKeyboardButton("👤 CEK PROFIL", callback_data="menu_profil")]
                    ])
                else:
                    notif_user += f"🔎 Sekarang kamu bisa langsung pakai fitur:\n"
                    notif_user += f"Klik menu *CARI DATA LAIN* di bawah! 👇\n"
                    kb_user = InlineKeyboardMarkup([
                        [InlineKeyboardButton("🔎 CARI DATA LAIN", callback_data="menu_cari_data")],
                        [InlineKeyboardButton("👤 CEK PROFIL", callback_data="menu_profil")]
                    ])
                
                await context.bot.send_message(chat_id=int(id_user), text=notif_user, parse_mode='Markdown', reply_markup=kb_user)
            except Exception as e:
                logger.error(f"Gagal notif user {id_user}: {e}")
                await update.message.reply_text(f"⚠️ Paket aktif tapi gagal kirim notif ke user {id_user} (mungkin belum /start): {e}", reply_markup=kb_admin_panel())

            context.user_data['mode'] = None
            context.user_data['paket_type'] = None
            return

        except Exception as e:
            logger.error(f"tambah paket error {e}")
            await update.message.reply_text(f"❌ Gagal: {e}\n\nFormat: `ID | HARI` contoh `123 | 30`", parse_mode='Markdown', reply_markup=kb_admin_panel())
            return
...
⚠️ Perhatian : untuk tetap waspada dan berhati-hati disarankan untuk rekber, terimakasih.sumber: https://t.me/Aakiwkiw_bot 🙏"""
    ...
    if context.user_data.get("awaiting_cari_lainnya"):
        ...
        for h in reversed(history[-10000:]):
            ...
            if query in txt_upper:
                hasil.append(h)
        # --- ANTI DUPLIKAT: maksimal 3 pesan per nomor yang sama ---
        filtered=[]
        seen={}
        for h in hasil:
            num = h.get("number","").strip()
            c = seen.get(num,0)
            if c < 3:
                filtered.append(h)
                seen[num]=c+1
...
async def cmd_cek_pantauan_debug(update, context):
    ...
    txt = f"🔧 DEBUG PANTAUAN V5 FINAL\nSupabase: {len(sup) if sup is not None else 'None/Error'} \nBackup file: {len(backup)} ({PANTAUAN_FILE})\nBackup persistent: {len(load_pantauan_backup())} ({PANTAUAN_FILE_PERSISTENT})\nMemory pantauan: {len(mem)}\nMemory blacklist: {len(bl)}\nGabungan get_all: {len(all_p)}\n\nFile exists: {exists1} {exists2}\nCWD: {_os.getcwd()}\nListdir: {str(_os.listdir('.'))[:500]}"
    await update.message.reply_text(txt, reply_markup=kb_main(uid))
