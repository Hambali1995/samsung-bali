...
# ========== TABEL PANTAUAN (RENAME DARI BLACKLIST) ==========
PANTAUAN_TABLE = "pantauan"
BLACKLIST_TABLE = PANTAUAN_TABLE  # Kompatibilitas lama, sekarang pakai pantauan

def load_pantauan_from_supabase():
    """Ambil semua nomor pantauan dari tabel khusus pantauan"""
    if not supabase_enabled():
        return None
    try:
        ...
        logger.error(f"PANTAUAN LOAD ERROR: {e}")
        # Fallback coba tabel lama blacklist jika pantauan belum ada
        try:
            ...
            r2.raise_for_status()
            rows2 = r2.json()
            numbers2 = [row.get("number") for row in rows2 if row.get("number")]
            return numbers2
        except:
            return None

def add_pantauan_to_supabase(number):
    """Tambah 1 nomor ke tabel pantauan"""
    if not supabase_enabled():
        return False
    try:
        url = f"{SUPABASE_URL}/rest/v1/{PANTAUAN_TABLE}"
        ...
        logger.error(f"PANTAUAN ADD ERROR {number}: {e}")
        return False

def remove_pantauan_from_supabase(number):
    """Hapus nomor dari tabel pantauan"""
    if not supabase_enabled():
        return False
    try:
        url = f"{SUPABASE_URL}/rest/v1/{PANTAUAN_TABLE}"
        ...
        r.raise_for_status()
        # Juga hapus dari tabel lama blacklist jika ada
        try:
            url2 = f"{SUPABASE_URL}/rest/v1/blacklist"
        ...
        logger.error(f"PANTAUAN REMOVE ERROR {number}: {e}")
        return False

def get_all_pantauan():
    """Ambil pantauan dari tabel khusus"""
    bl_new = load_pantauan_from_supabase()
    if bl_new is not None:
        return bl_new
    return db.get("blacklist", []) or db.get("pantauan", [])

# === KOMPATIBILITAS LAMA (ALIAS) ===
def load_blacklist_from_supabase():
    return load_pantauan_from_supabase()
def add_blacklist_to_supabase(number):
    return add_pantauan_to_supabase(number)
def remove_blacklist_from_supabase(number):
    return remove_pantauan_from_supabase(number)
...
def reset_old_blacklist():
    try:
        if "blacklist" in db and db["blacklist"]:
            db["blacklist"] = []
            save_db()
            logger.info("✅ Old blacklist migrated to pantauan")
    except Exception as e:
...
def whatsapp_webhook():
    try:
        ...
        # === CEK NO PANTAUAN - JANGAN FORWARD KE TELEGRAM BIAR TRANSAKSI AMAN ===
        # Cek dari Supabase pantauan + data lokal
        try:
            pantauan_list = get_all_pantauan()
        except:
            pantauan_list = []
        # Gabung dengan data lokal fresh_db untuk safety
        pantauan_list = list(pantauan_list or []) + list(fresh_db.get("blacklist", []) or []) + list(fresh_db.get("pantauan", []) or [])
        # Normalisasi nomor untuk cek (cek versi 0xxx dan 62xxx)
        cek_numbers = [sender_number_formatted, sender_number]
        # Tambah versi 0 -> 62 juga
        ...
        if is_pantauan:
            logger.info(f"🚫 NO PANTAUAN terdeteksi {sender_number_formatted} - TIDAK di-forward ke Telegram (transaksi aman)")
            return "ok", 200
...
def kb_hasil_cari_selesai():
    # Tombol untuk pesan SELESAI - ADA CARI DATA LAGI + KEMBALI MENU UTAMA
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔎 CARI DATA LAGI", callback_data="menu_cari_data")],
        [InlineKeyboardButton("🏠 KEMBALI MENU UTAMA", callback_data="back_main")]
    ])
...
❌ Ketik /batal untuk batal."""
    ...
    if data=="admin_hapus_pantauan" or data=="admin_hapus_blacklist":
        ...
        await context.bot.send_message(chat_id=uid, text="➖ HAPUS NO PANTAUAN\n\nKirim nomor yang ingin dihapus dari pantauan (bisa bulk):", reply_markup=kb_back_main_only())
        return
    if data=="admin_pantauan_menu" or data=="admin_blacklist_menu":
        if not is_admin(uid): return
        bl = db.get("pantauan", db.get("blacklist", []))
        txt=f"🚫 KELOLA NO PANTAUAN\n\n📊 Total: {len(bl)} nomor"
        kb=InlineKeyboardMarkup([
            [InlineKeyboardButton("➕ TAMBAH NO PANTAUAN (BULK)", callback_data="admin_tambah_pantauan"), InlineKeyboardButton("➖ HAPUS NO PANTAUAN", callback_data="admin_hapus_pantauan")],
            [InlineKeyboardButton("📋 LIHAT PANTAUAN", callback_data="menu_pantauan")],
...
async def text_handler(update,context):
    ...
    # === TAMBAH NO PANTAUAN BULK - support /Adds ===
    if text.upper().startswith("/ADDS") or text.upper().startswith("/ADD "):
        ...
        await update.message.reply_text(f"✅ Berhasil tambah {added} nomor ke PANTAUAN (dari {len(all_nums)} input)\n📊 Total sekarang: {total} nomor\n\nOtomatis masuk ke NO PANTAUAN ✅", reply_markup=kb_admin_panel())
        return
    ...
    if context.user_data.get("awaiting_cari_data"):
        ...
        history=load_wa_history()
        hasil=[]
        # Cari SEMUA pengirim yang mengandung kota tersebut
    ...
    if context.user_data.get("awaiting_cek_pantauan"):
        ...
        if len(clean)>=8:
            if clean in get_all_pantauan():
                await update.message.reply_text(f"🚫 Nomor {clean} ADA di PANTAUAN\n📊 Total: {len(db.get('blacklist',[]))} nomor", reply_markup=kb_blacklist_menu())
            else:
                await update.message.reply_text(f"✅ Nomor {clean} BELUM ADA di pantauan\n📊 Total: {len(db.get('blacklist',[]))} nomor", reply_markup=kb_blacklist_menu())
        else:
    ...
    if context.user_data.get("awaiting_hapus_pantauan"):
        ...
        for n in nums:
            ...
            if clean in existing:
                remove_pantauan_from_supabase(clean)
                if "pantauan" in db and clean in db["pantauan"]:
                    db["pantauan"].remove(clean)
                if "blacklist" in db and clean in db["blacklist"]:
                    db["blacklist"].remove(clean)
                removed+=1
        save_db()
        total = len(get_all_pantauan())
        await update.message.reply_text(f"✅ Berhasil hapus {removed} nomor dari PANTAUAN\n📊 Total: {total} nomor", reply_markup=kb_admin_panel())
        return
...
async def cmd_cek(update,context):
    ...
    if len(clean)>=8:
        if clean in get_all_pantauan():
            await update.message.reply_text(f"🚫 Nomor {clean} ADA di PANTAUAN kami 🚫\n📊 Total: {len(db.get('blacklist',[]))} nomor",reply_markup=kb_main(uid))
        else:
            await update.message.reply_text(f"✅ Nomor {clean} BELUM ADA di pantauan\n📊 Total: {len(db.get('blacklist',[]))} nomor",reply_markup=kb_main(uid))
    else:
